...
def test_opening_introduces_crew_briefly_before_lineup():
    ...
    assert len(lineup_segments) == 14
    assert lineup_segments[0].category == "opening_lineup_handoff"
    assert "send it to Jeff for the starting lineup" in lineup_segments[0].message
...
def test_opening_uses_custom_broadcaster_names():
    ...
    assert "22-car field" in segments[0].message
    assert "Continuing with the next ten in the lineup" in segments[10].message
