...
def test_best_render_template_prefers_same_stockcar_family_when_exact_model_is_missing(tmp_path):
    requests = find_render_requests_in_text(
        (
            "http://127.0.0.1:32034/pk_car.png?size=2&carPath=rt2000&number=64 "
            "http://127.0.0.1:32034/pk_car.png?size=2&carPath=stockcars%5Cchevyss&number="
        ),
        tmp_path / "data_1",
    )
    ...
    assert template.car_path == "stockcars chevyss"


def test_car_path_family_score_understands_next_gen_manufacturers():
    assert car_path_family_score("stockcars chevyss", "stockcars chevycamarozl12022") > 0
    assert car_path_family_score("stockcars fordfusion", "stockcars fordmustang2022") > 0
    assert car_path_family_score("stockcars toyotacamry", "stockcars toyotacamry2022") > 0
