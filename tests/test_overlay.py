...
def test_overlay_has_optional_ticker_leaderboard_and_compact_lap_bar():
    ...
    assert 'id="flo-track-text"' in OVERLAY_HTML
    assert 'setText("flo-track-text", state.track_name || "")' in OVERLAY_HTML
    assert ".flo-track-text" in OVERLAY_HTML
...
def test_overlay_has_brazen_leaderboard_style():
    ...
    assert "linear-gradient(118deg, transparent 0 14%, rgba(147, 51, 234" in OVERLAY_HTML
    assert ".brazen-cell::after" in OVERLAY_HTML
    assert "linear-gradient(122deg, transparent 0 28%" in OVERLAY_HTML
