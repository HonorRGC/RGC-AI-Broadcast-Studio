...
For post-race Discord reports, league profiles can use `league\race_schedule.csv` to map each track to its Sim Racer Hub race ID. Add one row per race with `track_name` and `schedule_id`; the Studio will match the current iRacing track and add the right Sim Racer Hub race-results link automatically. It also builds the championship standings link from the Season ID and race `schedule_id`. If Sim Racer Hub does not expose the schedule cleanly, enter the first race `schedule_id` in the Studio and the importer can build the rest sequentially.
...
Copy-Item league.example\race_schedule.csv league\race_schedule.csv
```
